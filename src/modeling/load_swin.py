...
from src.modeling.modeling_perceiver_xattn import Perceiver
...
class myVideoSwin(torch.nn.Module):
    def __init__(self, args, cfg, backbone):
        ...
        self.motion_perceiver = Perceiver(dim=512,k_v_dim=512,depth=1,dim_head=64,num_latents=64)
        self.appearance_perceiver = Perceiver(dim=512,k_v_dim=1024,depth=1,dim_head=64,num_latents=64)
        self.backbone = backbone
        self.use_grid_feature = args.grid_feat

    def forward(self, x, need_stage=False):
        x = self.backbone(x,need_stage)
        if type(x) == tuple:
            x, stages_feature = x[0],x[1]
            B,D,_,_,_ = stages_feature[1].shape
            x_motion = self.motion_perceiver(stages_feature[1].permute(0,2,3,4,1).reshape(B,-1,D)).squeeze()
            B,D,_,_,_ = x.shape
            x_appearance = self.appearance_perceiver(x.permute(0,2,3,4,1).reshape(B,-1,D)).squeeze()
            x = torch.cat([x_motion,x_appearance],dim=1)
        return x